...
module.exports = {
  ...
  question15: async (req, res, next) => {
    try {
      let { supplierNames } = req.query; // ['nccs1', 'nccs2', 'nccs3']
  ...
  question18: async (req, res, next) => {
    try {
      ...
      //   .lookup({
      //     from: 'categories',
      //     localField: 'categoryId', // TRUY VẤN NGƯỢC!!!
      //     foreignField: '_id',
      //     as: 'categories',
      //   })
      ...
      let results = await Category.aggregate()
        .lookup({
          ...
          foreignField: 'categoryId', // Teen field trong form
          as: 'products',
        })
        // .unwind('products') // sẽ dẫn dến thiếu dự liệu trừ trường hợp chắc chắn có 1 hoặc nhiều products, Loai bỏ trường hợp không có sản phẩm.
        .unwind({
          path: '$products',
